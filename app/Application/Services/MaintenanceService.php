...
class MaintenanceService {
    ...
    public function create(array $data,int $actorId): int {
        $eid=(int)($data['equipment_id']??0);$qty=(int)($data['quantity']??0);if($qty<1)throw new RuntimeException('Maintenance quantity must be at least 1.');
        ...
        try{
            if(!$this->repository->lockEquipment($eid))throw new RuntimeException('Equipment not found.');if($qty>$this->repository->availableQuantity($eid))throw new RuntimeException('Maintenance quantity exceeds currently available equipment.');
            $data['reported_by']=$actorId;$model=new MaintenanceRecordModel();$model->insert($data);$id=(int)$model->getInsertID();$this->audit->log($actorId,'maintenance_created','maintenance_record',$id,'Created maintenance record');
            if($db->transStatus()===false)throw new RuntimeException('Maintenance record could not be created.');$db->transCommit();return $id;
